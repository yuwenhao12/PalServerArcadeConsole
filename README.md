...
![PalServer Arcade Console 控制台总览](docs/assets/console-overview-20260712.png)

### 规则面板

![PalServer Arcade Console 规则面板](docs/assets/console-rules-20260712.png)

### 世界图鉴：帕鲁

![PalServer Arcade Console 世界图鉴帕鲁页](docs/assets/world-atlas-pals-20260712.png)

### 世界图鉴：背包与装备

![PalServer Arcade Console 世界图鉴背包与装备页](docs/assets/world-atlas-inventory-20260712.png)
...
确认宿主机能看到 Palworld 容器：

```bash
docker ps --format '{{.Names}}'
```

输出中应包含 `.env` 中 `PALWORLD_DOCKER_CONTAINER` 配置的容器名。同时确认 Compose 中保留了 `/var/run/docker.sock:/var/run/docker.sock` 挂载。
...
镜像中的系统依赖通过 Debian 官方源安装，存档解析所需的 `loguru`、`orjson` 与 `pyooz` 通过 PyPI 安装。容器重启功能直接调用挂载的 Docker Socket，不再安装完整 Docker CLI。若 apt 网络超时，可保留 `.env` 中的 `APT_MIRROR`，然后重新执行上述构建命令。
